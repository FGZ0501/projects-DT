        ...
        <div class="zhuce-zhuce">
            <div class="zhuce-left">
                <div class="zhuce-box">
                    ...
                    <div class="zhuce-l">
                        <input type="password" placeholder="请设置密码" style="width:281px;height:37px;font-size: 18px;" v-model='passWord'>
                    </div>
                    <div class="zhuce-l">
	                ...
	                if(this.checkPhone()){//判断手机号码是否符合规则
	                    if(this.imgCode==''){
	                        ...
	                        this.ajax.post('/xinda-api/register/sendsms',qs.stringify({
	                            ...
	                            if(res.data.status!=-1){//判断验证码是否填写正确
	                            	this.judgeclass = false;//设置按钮为点击后的样式
		                            this.getnum = this.num+"s后重新获取"//改变按钮文字内容
		                            var that = this;
		                            this.judge = false;
		                            for (var i = 59;i > 0;i--){//闭包定时器不让验证码重复发送
		                            	(function (i){
			                            	setTimeout(function(){
			                            		that.getnum = that.num-- +"s后重新获取";
			                            	},1000*(i-1))
		                            	})(i)
		                            };
		                            setTimeout(function(){ //重置文字内容和样式
		                            	that.judge = true;
		                            	that.num =59;
		                            	that.getnum = "点击获取";
